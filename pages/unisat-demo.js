...
/* ========= Main Page Component ========= */
export default function UnisatDemoPage() {
...
3) When a game is over, press Space to restart.`;
  ...
  // Step 1: Connect using Unisat Wallet API.
  async function handleConnectUnisat() {
    if (typeof window.unisat === 'undefined') {
      setDebugMsg('Unisat Wallet is not installed.');
      return;
    }
    try {
      setDebugMsg('Connecting to Unisat Wallet...');
  ...
  // Step 2: Fetch inscriptions (or fallback identifier) from Unisat Wallet.
  ...
  return (
    <div style={{ padding: '1rem' }}>
      <h2>Unisat Wallet Maze & Snake Demo</h2>
      <pre style={{ background: '#222', color: '#fff', padding: '0.5rem' }}>
        {instructions}
      </pre>
      <p>
        This demo uses the Unisat Wallet API to connect, fetch your asset identifiers, and generate
        deterministic games from them.
      </p>

      {/* Step 1: Connect */}
      <div style={{ marginBottom: '1rem' }}>
        <h3>1) Connect to Unisat Wallet</h3>
        <button onClick={handleConnectUnisat} style={{ padding: '0.5rem 1rem' }}>
